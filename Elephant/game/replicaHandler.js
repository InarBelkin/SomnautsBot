...
export function getCurrentReplica(args) {
    return {
...
export function nextReplica(args) {
    if (typeof args.answerId != "string" && typeof args.answerText != "string")
        throw TypeError("at least one of answerId or answerText must be a string");
    const mes = typeof args.answerId == "string" ? answers.find(value => value.id == args.answerId).text : args.answerText;
    if (mes == undefined)
        throw TypeError("index of answer was incorrect");
